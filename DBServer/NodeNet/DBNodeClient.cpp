...
{
    m_pNetClientModule->NFINetClientModule::AddEventCallBack(EServerType::SERVER_TYPE_MASTER, this, &DBNodeClient::OnSocketEvent);
	m_pNetClientModule->NFINetClientModule::AddEventCallBack(EServerType::SERVER_TYPE_GATE, this, &DBNodeClient::OnSocketEvent);//world服连接master服务
...
void DBNodeClient::SetServerInfoReport()
{
    ...
    m_serverInfoReport.set_server_type(EServerType::SERVER_TYPE_DB);
}
...
void DBNodeClient::OnSocketEvent(const NFSOCK nSockIndex, const NF_NET_EVENT nEvent, NFINet* pNet)
{
